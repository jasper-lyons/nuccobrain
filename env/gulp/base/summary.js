...
function getFilepack() {
	const filepack = path.join(config('paths.public'), 'filepack.zip').replace(/\\/, '/');
...
function getHtmlPages() {
	const files = fs.readdirSync(config('paths.public'));
	const htmlFiles = [];
 ...
 *
 * @return {object|bool} - true if there are no errors, 
 * false if report file is missing, object otherwise.
 */

function getJsErrors() {
	let jsReport = 'reports/eslint-from-gulp.json';
